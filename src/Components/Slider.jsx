...
const Slider = () => {
  ...
  return (
    ...
    >
      ...
      <div
        ref={sliderRef}
        className="relative w-[100vw] overflow-hidden rounded-lg md:h-[650px]"
      ...
      >
        {data.map((item, index) => (
          ...
          >
            {/* Gradient Divs */}
            <div
              className="absolute z-30 top-0 left-0 w-[100vw] h-full pointer-events-none"
              style={{
                background : " linear-gradient(0deg, rgba(0, 0, 0, 0.7) 0%, rgba(0, 0, 0, 0.2) 20%, rgba(0, 0, 0, 0.1) 40%, rgba(0, 0, 0, 0) 60%, rgba(0, 0, 0, 0.4) 80%, rgba(0, 0, 0, 0.7) 100%)"
              }}
            ></div>
            <div
              className="absolute z-40 top-0 left-0 w-[100vw] h-full pointer-events-none"
              style={{
            ...
            {/* Slide Content */}
            <div className="absolute w-[58%] z-50 bottom-14 left-0 right-0 p-10 text-white">
              <span className="font-Mons ">
                #{item.spolightNumber} Spotlight
              </span>
              <h2 className="text-6xl font-bebas">{item.title}</h2>
              <div className='flex items-center jc'>
               <span className=" flex my-2 gap-2 font-bold  ">
              • <span className='text-[#ffffffb3] flex font-Mons text-sm'>{item.type } <pre>  </pre>  {item.episodeLength } </span> •
              </span> 
              </div>
              <p className='text-[#ffffffb3] font-bold flex font-Mons text-[16px]'>{item.Name}</p>
              <p className="text-[#ffffffb3] flex font-Mons mt-2 s text-[15px] font-bold">
                {truncateSummary(item.summery, 200)} {/* Assuming 150 characters for two lines */}
              </p>
              <div className="btns flex pt-6">
      ...
      {/* Pagination Dots */}
      <div className="absolute top-48 rotate-90 cursor-pointer pointer-events-auto -right-[80px] flex justify-center space-x-5">
        {data.map((_, index) => (
          <button
            key={index}
            onClick={() => goToSlide(index)}
            className={`w-[17px] h-[17px] rounded-full transition-colors duration-300 ${
      ...
      {/* Carousel controls */}
      <button
        className="absolute top-[40%] right-4 z-30 flex items-center justify-center h-full px-4 cursor-pointer pointer-events-auto"
        onClick={prevSlide}
        data-carousel-prev
      >
        <span className="inline-flex items-center justify-center transition-colors duration-100 hover:bg-[#1BE887] w-12 h-12 rounded-full bg-[#ffffff1a]">
          <svg
            aria-hidden="true"
      ...
      <button
        className="absolute top-[32%] right-4 z-30 flex items-center justify-center h-full px-4 cursor-pointer pointer-events-auto"
        onClick={nextSlide}
        data-carousel-next
      >
        <span className="inline-flex items-center justify-center transition-colors duration-100 hover:bg-[#1BE887] w-12 h-12 rounded-full bg-[#ffffff1a]">
